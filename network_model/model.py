...
class Model(object):
    ...
    def build_network_nips(self, name, trainable, num_actions):
        ...
        with tf.variable_scope(name):
            ...
            self.h_conv1 = tf.nn.relu(tf.nn.conv2d(x_normalized, W_conv1, strides=[1, 4, 4, 1], padding='VALID') + b_conv1, name="h_conv1")
            self.print_log(self.h_conv1)
            ...
            self.h_conv2 = tf.nn.relu(tf.nn.conv2d(self.h_conv1, W_conv2, strides=[1, 2, 2, 1], padding='VALID') + b_conv2, name="h_conv2")
            self.print_log(self.h_conv2)
    
            conv_out_size = np.prod(self.h_conv2._shape[1:]).value
    
            h_conv2_flat = tf.reshape(self.h_conv2, [-1, conv_out_size], name="h_conv2_flat")

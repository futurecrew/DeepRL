...
class Model(object):
    def __init__(self, args, name, action_no, thread_no):
        self.args = args
        ...
        if args.use_color_input:
            ...
            self.input_channel_no = self.history_len
            
        with tf.device(args.device):
            self.build_network(name, args.network, action_no)
    
    def make_layer_variables(self, shape, name_suffix, weight_range=-1):
        if weight_range == -1:
            weight_range = 1.0 / math.sqrt(np.prod(shape[0:-1]))
        weights = tf.Variable(tf.random_uniform(shape, minval=-weight_range, maxval=weight_range), name='W_' + name_suffix)
        biases  = tf.Variable(tf.random_uniform([shape[-1]], minval=-weight_range, maxval=weight_range), name='b_' + name_suffix)
    ...
    def build_network(self, name, network, num_actions):
        net_funct = getattr(self, 'build_network_' + self.args.network)
        net_funct(name, num_actions)
        
    def build_network_nature(self, name, num_actions):
        self.print_log("Building network nature for %s" % (name))
    ...
    def build_network_nips(self, name, num_actions):
        self.print_log("Building network nips for %s" % (name))
